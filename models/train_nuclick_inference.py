...
def ROI_crop_signal_line(aux_input, mask):
    # 假设 input 和 aux_input 的形状分别为 (batch_size, 3, H, W) 和 (batch_size, 2, H, W)
    ...
    aux_input_new = torch.zeros(batch_size, 2, H, W).to(aux_input.device)
    ...
    for b in range(batch_size):
        ...
        guidingSignal = generateGuidingSignal(roi_mask_new, RandomizeGuidingSignalType)
        ...
        aux_input_new[b, :, start_y:end_y, start_x:end_x] = guidingSignal_expanded
    ...
    # return roi_inputs, roi_aux_inputs, roi_suppixels , roi_masks, mask_box
    return aux_input_new
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, mask, signal, filename
...
# i = 1
# cls = 'necrosis'
# train_images_dir = f"/data_nas/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/step256_patch512_nofilling/fold_{i}/train/{cls}/image_npy"
# train_masks_dir = f"/data_nas/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/step256_patch512_nofilling/fold_{i}/train/{cls}/mask_npy"
# train_signal_dir = f'/data_nas/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/step256_patch512_nofilling/fold_{i}/train/{cls}/signal_max_point_npy'
# val_images_dir = f"/data_nas/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/step256_patch512_nofilling/fold_{i}/val/{cls}/image_npy"
# val_masks_dir = f"/data_nas/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/step256_patch512_nofilling/fold_{i}/val/{cls}/mask_npy"
# val_signal_dir = f'/data_nas/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/step256_patch512_nofilling/fold_{i}/val/{cls}/signal_max_point_npy'
...
# Val_signal_png_dir = f'/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/val/{cls}/signal_max_line_png'
...
model.load_state_dict(torch.load('/home/gjs/ISF_nuclick/check_points_BCSS/nuclick_512_all_class_100.pth'))
...
def train_model(model, train_loader, val_loader, loss_fn, optimizer,  epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for item,(images, masks, aux_inputs) in enumerate(train_loader):
            ...
            aux_input_new= ROI_crop_signal_line(aux_inputs, masks)
            
            input = torch.cat([images, aux_input_new], dim=1)
        ...
        with torch.no_grad():
            iou_scores = []
            for images, masks, aux_inputs , filenames in val_loader:
                images, aux_inputs, masks = images.to(device), aux_inputs.to(device), masks.to(device)
                aux_input_new= ROI_crop_signal_line(aux_inputs, masks)
                # first_channel = aux_input_new[:, 0, :, :]  # 提取第一个通道
                
                # for i in range(aux_input_new.size(0)):
                #     # 选择一个批次的张量（2, H, W），并将其转换为 NumPy 数组
                #     filename = filenames[i]
                #     image_tensor = first_channel[i].cpu().numpy()
                #     image = np.clip(image_tensor * 255, 0, 255).astype(np.uint8)  # 将像素值缩放并转为整数

                #     # 使用 PIL 保存图像
                #     image_pil = Image.fromarray(image)
                #     image_pil.save(os.path.join(Val_signal_png_dir, f"{filename}.png"))
        ...
        if dice_score > best_dice:
            ...
            # torch.save(model.state_dict(), f'/home/gjs/ISF_nuclick/check_points_BCSS/nuclick_512_{cls}_100.pth')


train_model(model, train_loader, val_loader, loss_fn, optimizer,  epochs=1)
# print("模型：", model.__class__.__name__)

print(f"nuclick_{cls}.")
