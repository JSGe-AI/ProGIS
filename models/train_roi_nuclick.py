...
class ResNetUNet_proto(nn.Module):
    ...
    def forward(self, roi_input , roi_aux_input , images, aux_inputs, masks):
...
def ROI_crop_signal_line(input, aux_input, mask):
    # 假设 input 和 aux_input 的形状分别为 (batch_size, 3, H, W) 和 (batch_size, 2, H, W)
    batch_size, _, H, W = input.shape
...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, images_dir, signal_dir, masks_dir,  filenames):
...
train_images_dir = f"/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/train/{cls}/1_image_npy"
train_masks_dir = f"/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/train/{cls}/1_mask_npy"
train_signal_dir = f'/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/train/{cls}/1_signal_max_point_npy'


val_images_dir = f"/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/val/{cls}/1_image_npy"
val_masks_dir = f"/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/val/{cls}/1_mask_npy"
val_signal_dir = f'/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/val/{cls}/1_signal_max_point_npy'
...
# 创建自定义数据集类的实例
train_dataset = CustomDataset(train_images_dir, train_signal_dir, train_masks_dir, train_filenames)
val_dataset = CustomDataset(val_images_dir, val_signal_dir, val_masks_dir, val_filenames)
...
model.load_state_dict(torch.load('/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/z_checkpoint_125WSI/nuclick_256_all_class_line_best_0-998.pth'))
...
def train_model(model, train_loader, val_loader, loss_fn, optimizer,  epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for item,(images, aux_inputs, masks) in enumerate(train_loader):
            ...
            roi_input , roi_aux_input , roi_mask = ROI_crop_signal_line(images , aux_inputs, masks)
            ...
            outputs = model(roi_input , roi_aux_input)
            
            # outputs = model(roi_input , roi_aux_input , images, aux_inputs, masks)
           
            
            loss = dice_loss(outputs, roi_mask)
            ...
            train_dice_score += dice_coeff(outputs, roi_mask).item() * images.size(0)
            
            # 计算准确率
            _, batch_accuracy = calculate_binary_segmentation_accuracy(outputs,roi_mask)
        ...
        with torch.no_grad():
            ...
            for images, aux_inputs, masks in val_loader:
                ...
                outputs = (outputs >= 0.8).int()
                
                dice_score += dice_coeff(outputs, roi_mask).item() * images.size(0)
                ...
                for pred, mask in zip(preds, roi_mask):
        ...
        if dice_score > best_dice:
            ...
            torch.save(model.state_dict(), f'/data_nas2/gjs/ISF_pixel_level_data/BCSS_x10_reinhard_cut/150/z_checkpoint_125WSI/nuclick_256_all_class_line_best_0-998_200epoch.pth')
