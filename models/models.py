...
def get_MultiScale_ResUnet (input_shape, lossType):
    if multiGPU:
        with tf.device("/cpu:0"):
            ...
            # conv10_class1 = Conv2D(1, (1, 1), activation='sigmoid')(conv9) # 修改添加多个分支。
            # conv10_class2 = Conv2D(1, (1, 1), activation='sigmoid')(conv9)
            # conv10_class3 = Conv2D(1, (1, 1), activation='sigmoid')(conv9)
            # conv10_class4 = Conv2D(1, (1, 1), activation='sigmoid')(conv9)
        
            # model = Model(inputs=[img, auxInput], outputs=[conv10_class1,conv10_class2,conv10_class3,conv10_class4])
            ...
            model.compile(optimizer=Adam(lr=learningRate), loss='binary_crossentropy', metrics=[dice_coef])#修改
            return model
    else:
        img = Input(input_shape + (img_chls,), name='main_input')  # size: 1024
        auxInput = Input(input_shape + (3,), name='dists_input')
