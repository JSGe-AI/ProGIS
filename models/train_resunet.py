...
class MultiClassSegmentationLoss(nn.Module):
    ...
    def focal_loss(self, pred, target, alpha=None, gamma=2):
        """
        ...
        if alpha is None:
            ...
            class_pixels = [130984493, 111602043, 39394529, 23730154, 18036621] #你的类别像素数量
...
def calculate_metrics(pred, target, num_classes):
    ...
    present_classes = np.unique(target) # 获取当前图片中存在的类别
    for cls in range(num_classes):
        if cls in present_classes:
            pred_cls = (pred == cls).astype(np.int32)
            target_cls = (target == cls).astype(np.int32)
...
model.load_state_dict(torch.load("/home/gjs/ISF_nuclick/check_points_BCSS/resunet_best_24.pth"))
optimizer = optim.Adam(model.parameters(), lr=1e-4)

class_pixels = np.array([130984493, 111602043, 39394529, 23730154, 18036621])  # 示例：四个类别的像素数量
...
for epoch in range(num_epochs):
    ...
    if val_dice.mean() > best_dice:
        ...
        filename = f'/home/gjs/ISF_nuclick/check_points_BCSS/resunet_best_1.pth'
        print(f"Epoch {epoch+1}: Model saved with best Val Dice: {val_dice.mean():.4f}")
#     # else:
#     #     filename = f'/home/gjs/ISF_nuclick/checkpoints_new/resunet_bcss_x10_reinhard_no_filling/resunet_allclass_x10_reinhard_changemodel_{epoch+1}.pth'
#     #     print(f"Epoch {epoch+1}: Model saved at interval.")
