...
class EfficientUnet(nn.Module):
    def __init__(self, encoder, out_channels=2, concat_input=True, backbone=True):
        super().__init__()
        ...
        if self.backbone:
            ##############  backbone 修改 ############
            self.conv_proh1 = nn.Conv2d(32, 32, 1)
            #########################################
    ...
    def size(self):
        ...
        if self.backbone == True:
            ...
            # 分割头
            size_dict = {'efficientnet-b0': [592, 296, 152, 80, 38, 32], 'efficientnet-b1': [592, 296, 152, 80, 35, 32],
                        'efficientnet-b2': [600, 304, 152, 80, 35, 32], 'efficientnet-b3': [608, 304, 160, 88, 35, 32],
                        'efficientnet-b4': [624, 312, 160, 88, 35, 32], 'efficientnet-b5': [640, 320, 168, 88, 35, 32],
                        'efficientnet-b6': [656, 328, 168, 96, 35, 32], 'efficientnet-b7': [672, 336, 176, 96, 35, 32]}
    ...
    def forward(self, x):
        input_ = x #[b, 3 , 256 , 256]

        blocks = get_blocks_to_be_concat(self.encoder, x)
        _, x = blocks.popitem() #[b, 1280 , 8 , 8]

        x = self.up_conv1(x) # [b, 512, 16, 16]
        x = torch.cat([x, blocks.popitem()[1]], dim=1)# [b, 592, 16, 16]
        x = self.double_conv1(x)# [b, 512, 16, 16]

        x = self.up_conv2(x)# [b, 256, 32, 32]
        x = torch.cat([x, blocks.popitem()[1]], dim=1)# [b, 296, 32, 32]
        x = self.double_conv2(x)# [b, 256, 32, 32]

        x = self.up_conv3(x)# [b, 128, 64, 64]
        x = torch.cat([x, blocks.popitem()[1]], dim=1)# [b, 152, 64, 64]
        x = self.double_conv3(x)# [b, 128, 64, 64]

        x = self.up_conv4(x)# [b, 64, 128, 128]
        x = torch.cat([x, blocks.popitem()[1]], dim=1)# [b, 80, 128, 128]
        x = self.double_conv4(x)# [b, 64, 128, 128]

        if self.concat_input:
            x = self.up_conv_input(x)# [b, 32, 256, 256]
            x = torch.cat([x, input_], dim=1)# [b, 35, 256, 256]
            x = self.double_conv_input(x)# [b, 32, 256, 256]
...
def get_efficientunet_b0(backbone=True, out_channels=2, concat_input=True, pretrained=True):
    encoder = EfficientNet.encoder('efficientnet-b0', pretrained=pretrained, backbone=backbone)
    model = EfficientUnet(encoder, out_channels=out_channels, concat_input=concat_input, backbone=backbone)
    return model
