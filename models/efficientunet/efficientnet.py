...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args_list, global_params, backbone=True):
        super().__init__()
    ...
    @classmethod
    def from_name(cls, model_name, *, n_classes=1000, pretrained=False, backbone = True):
        return _get_model_by_name(model_name, classes=n_classes, pretrained=pretrained, backbone=backbone)

    @classmethod
    def encoder(cls, model_name, *, pretrained=False, backbone = True):
        model = cls.from_name(model_name, pretrained=pretrained, backbone=backbone)
...
def _get_model_by_name(model_name, classes=1000, pretrained=False, backbone=True):
    block_args_list, global_params = get_efficientnet_params(model_name, override_params={'num_classes': classes})
    model = EfficientNet(block_args_list, global_params, backbone=backbone)
    try:
        if pretrained:
