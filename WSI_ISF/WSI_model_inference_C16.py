...
def processMasks(pred_mask_all, GT_mask_all):
    ...
    fg_all = (GT_mask_all == 1) & (pred_mask_all == 0) 
    bg_all = (GT_mask_all == 0) & (pred_mask_all == 1) 
...
class SimilarityModel(nn.Module):
    ...
    def forward(self, roi_signal, roi_features, roi_mask):
        ...
        segmentation_map = self.classification_head(attention_output).float()
...
def compute_iou(pred, target, cls):
    ...
    return float('nan') if union == 0 else intersection / union
...
def calculate_accuracy(pred, target, coordinates):
    ...
    return (TP + TN) / total_pixels if total_pixels > 0 else 0
...
# 推理与验证流程 (Pipeline)
# ==============================================================================
def preprocess_and_inference(fold, ckpt, threod, val_h5_dir, val_signal_dir, val_mask_dir, vis_save_path, num_epochs=1, learning_rate=1e-4, batch_size=1, device='cuda:1', feature_dim=512, inter_steps=20):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for coordinates, features, signal_coords, mask, signal_file in val_loader_tqdm:
                ...
                out_mask = torch.zeros((batch_size, 1, H, W), device=mask.device)
                ...
                with autocast(): 
                    ...
                    while count < inter_steps:
                        ...
                        segmentation_map[:, :, start_x: end_x, start_y: end_y] = roi_segmap
                        ...
                        out_mask_list.append(out_mask.clone())
                        count += 1 
                ...
                # --- 保存可视化结果 ---
                filename = os.path.basename(signal_file[0])
                name = os.path.splitext(filename)[0]

                pre_mask = mask.squeeze(0).detach().cpu().numpy()
                pre_mask = np.swapaxes(pre_mask, 0, 1)
                
                foreground_rgb = (64, 128, 0) 
                background_rgb = (0, 64, 128) 
                rgb_mask = np.zeros((pre_mask.shape[0], pre_mask.shape[1], 3), dtype=np.uint8)
                rgb_mask[pre_mask == 1] = foreground_rgb
                rgb_mask[pre_mask == 0] = background_rgb

                image = Image.fromarray(rgb_mask)
                os.makedirs(vis_save_path, exist_ok=True)
                image.save(os.path.join(vis_save_path, f"{name}.png"))
...
class Config:
    # 1. 基础路径配置
    BASE_DATA_PATH = "/data_nas2/gjs/Camelyon16/conch_5_fold"
    CKPT_TEMPLATE = "WSI_ISF/conch_5_fold_ckpt/C16/C16_best_model_fold_{}.pth"
    SAVE_VIS_PATH = "WSI_ISF/Visualization_conch_ours_C16_mask"
    
    # 2. 硬件与执行范围配置
    DEVICE = 'cuda:4'
    START_FOLD = 2  # 循环开始的折数
    END_FOLD = 6    # 循环结束的折数（不包含该值，即运行 2, 3, 4, 5）
...
if __name__ == "__main__":
    for i in range(Config.START_FOLD, Config.END_FOLD):
        print(Config.BASE_DATA_PATH)
        print("fold", i)
        
        # 动态组装当前 fold 的路径
        val_h5_dir = f'{Config.BASE_DATA_PATH}/fold_{i}/test/feature_h5'
        val_signal_dir = f'{Config.BASE_DATA_PATH}/fold_{i}/test/signals_max_XY'
        val_mask_dir = f'{Config.BASE_DATA_PATH}/fold_{i}/test/mask'
        ckpt_path = Config.CKPT_TEMPLATE.format(i)

        preprocess_and_inference(
            fold=i, 
            ckpt=ckpt_path, 
            threod=Config.THRESHOLD, 
            val_h5_dir=val_h5_dir, 
            val_signal_dir=val_signal_dir, 
            val_mask_dir=val_mask_dir, 
            vis_save_path=Config.SAVE_VIS_PATH,
            device=Config.DEVICE, 
            num_epochs=Config.NUM_EPOCHS, 
            feature_dim=Config.FEATURE_DIM,
            learning_rate=Config.LEARNING_RATE,
            batch_size=Config.BATCH_SIZE,
            inter_steps=Config.inter_steps_num
        )