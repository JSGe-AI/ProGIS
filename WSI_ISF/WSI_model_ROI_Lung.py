...
def cut_roi(coordinates, features, signal_coords, mask):
    ...
    if signal_coords.size(1) > 0:
        ...
        x_min_1 = torch.tensor(0, device=signal_coords.device, dtype=signal_coords.dtype)
    ...
    if end_y > W:
        ...
        start_y = torch.tensor(max(end_y - 100, 0), dtype=torch.float)
    if end_x > H:
        end_x = torch.tensor(H, dtype=torch.float)
        start_x = torch.tensor(max(end_x - 100, 0), dtype=torch.float)
...
def preprocess_and_train(path, fold, train_h5_dir, train_signal_dir, train_mask_dir, val_h5_dir, val_signal_dir, val_mask_dir, 
                         num_epochs=30, learning_rate=1e-4, batch_size=1, device='cuda:1', feature_dim=1024, num_workers=8, save_prefix='Lung'):
    best_roi_dice = 0
    ...
    for epoch in range(num_epochs):
        ...
        for coordinates, features, signal_coords, mask, signal_file in train_loader_tqdm:
            ...
            signal_coords = signal_coords.to(device, non_blocking=True)
        ...
        if val_mean_dice_roi > best_roi_dice:
            ...
            torch.save(model.state_dict(), f'{path}/{save_prefix}_best_model_fold_{fold}.pth')
...
if __name__ == "__main__":
    ...
    # ---------------- 1. 基础路径与设备配置 ----------------
    BASE_PATH = "/data_993/gjs/UNI_5_fold"              # 数据集总路径
    FOLD_LIST = [4, 5]                                  # 需要运行的交叉验证折数列表
    DEVICE = "cuda:3"                                   # 训练设备 (比如 'cuda:0', 'cuda:1')
    MODEL_SAVE_PREFIX = "Lung"                          # 模型保存时的命名前缀
    ...
    NUM_WORKERS = 8                                     # DataLoader加载数据的进程数
